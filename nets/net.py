...
class Net(object):
    ...
    def save(self,dir,step=None):
        ...
        if(step is not None):
            self.saver.save(sess,dir+'/model.ckpt',global_step=step,write_meta_graph=False)
        else :
            self.saver.save(sess,dir+'/last.ckpt',write_meta_graph=False)
    ...
    @staticmethod
    def _build_net(spec,_t,is_training=False):
        for block in spec:
