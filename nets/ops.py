...
class TransposedConv2d(object):
    def __init__(self,name,input_dim,out_dim,
                 k_h=4,k_w=4,d_h=2,d_w=2,stddev=0.02,data_format='NCHW',padding='SAME') :
        with tf.variable_scope(name) :
    ...
    def __call__(self,input_var,name=None,**xargs):
        bs = tf.shape(input_var)[0]
        shapes = input_var.get_shape().as_list()

        h,w = shapes[2:4] if self.data_format == 'NCHW' else shapes[1:3]
        d_h,d_w = self.strides[2:4] if self.data_format == 'NCHW' else self.strides[1:3]
        k_h,k_w,out_dim,_ = self.w.get_shape().as_list()

        if self.padding == 'SAME':
            out_h,out_w = h*d_h, w*d_w
        elif self.padding == 'VALID':
            out_h = ((h-1)*d_h + k_h)
            out_w = ((w-1)*d_w + k_w)

        if( self.data_format == 'NCHW' ):
            shapes = tf.stack([bs,out_dim,out_h,out_w])
        else:
            shapes = tf.stack([bs,out_h,out_w,out_dim])
